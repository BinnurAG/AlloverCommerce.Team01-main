...
public class TC02 extends ReusableMethods {
    private final String testName = "US18 / TC02-Description Text Box";
    private final String description = "Data must be entered in the Description text box.(PASSED)";
    private final String reportMessage = "It has been verified that data can be entered in the Description text box..";
    ...
    public void tc02() {
        ...
        descriptionMessage.sendKeys(ConfigReader.getProperty("descriptonMes"));
        extentTest.info("Description section is filled.");
        actions.sendKeys(Keys.PAGE_DOWN).perform();
        waitForSecond(2);

        Assert.assertEquals(p20VendorCouponsPage.descriptionMessage.getAttribute("value"), ConfigReader.getProperty("descriptonMes"));
