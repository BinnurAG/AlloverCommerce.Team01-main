...
public class TC08 extends ReusableMethods {
    private final String testName = "US18 / TC08-Show On Store";
    private final String description = "Show on store checkbox must be selectable.(PASSED)";
    private final String reportMessage = "It has been verified that the Show on store checkbox is selectable.";
    ...
    public void tc01() {
        ...
        ReusableMethods.click(p20VendorCouponsPage.showOnStore);
        extentTest.pass("Show on store box selected.");

        actions.sendKeys(Keys.PAGE_DOWN).perform();

        waitForSecond(2);
        Assert.assertTrue(p20VendorCouponsPage.showOnStore.isSelected());
        extentTest.info(reportMessage);
        ReusableMethods.scroll(p20VendorCouponsPage.showOnStore);
