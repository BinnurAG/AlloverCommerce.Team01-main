...
public class TC07 extends ReusableMethods {
    private final String testName = "US18 / TC07-Allow Free Shipping";
    private final String description = "Allow free shipping checkbox must be selectable.(PASSED)";
    private final String reportMessage ="It has been verified that the Allow free shipping checkbox can be selected.";
    ...
    public void tc01() {
        ...
        ReusableMethods.click(p20VendorCouponsPage.allowFreeShipping);
        extentTest.info("Allow Free Shipping box is checked.");

        actions.sendKeys(Keys.PAGE_DOWN).perform();

        waitForSecond(2);
        Assert.assertTrue(p20VendorCouponsPage.allowFreeShipping.isSelected());
